(function( $, undefined ) {
	...
	$.JMSlideshow.prototype 	= {
		_init 				: function( options ) {
			...
			// step's bgcolor
			this.colors			= $.map( this.$slides, function( el, i ) { return $( el ).data( 'image' ); } ).join( ' ' );
		...
		_initImpress		: function() {
			...
			this.$jmsWrapper.jmpress( 'setActive', function( slide, eventData ) {
				...
				// add bg color class
				if($('#jms-bg-'+slide.index()).length){
					$('.jms-bg').css({"z-index":8})
					$('#jms-bg-'+slide.index()).css({"z-index":10})
					$(".jms-bg").stop(true, true).fadeOut();
					$('#jms-bg-'+slide.index()).fadeIn();
				}else{
				}
			...
			$("#jms-slideshow .step").each(function() {
				...
				$("#jms-slideshow").append('<img src="'+imagelink+'" id="jms-bg-'+index+'" height="'+$(this).attr("data-image-h")+'" width="'+$(this).attr("data-image-w")+'" class="jms-bg" alt="">');
				$(".jms-bg").hide();
			});
			$("#jms-bg-0").fadeIn();
		},
