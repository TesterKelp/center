...
您现在的位置：<a href="/">首页</a> >> <a href="/${m.item_directorie}/portal.do">${m.item_name}  </a>>> ${c.catename}
...
<c:forEach items="${lst}" var="a" varStatus="status">
 ...
 <c:if test="${ empty cat}">
                ...
                <div class="box" style="height:65px;line-height:20px;"><c:if test="${fn:length(a.articleitd)>100 }">${ fn:substring( a.articleitd ,0,100)} ...</c:if>
        <c:if test="${fn:length(a.articleitd)<=100 }">${  a.articleitd }</c:if></div>
                <hr style="height:1px;border:0;border-bottom:1px dashed #ccc;">
