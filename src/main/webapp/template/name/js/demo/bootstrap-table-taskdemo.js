...
//      url: "../../test/js/demo/bootstrap_table_test.json",
      url: "http://192.168.1.100:8080/module/defect/JstaskList.do", 
      search: true,
      pagination: true,
      pageSize: 20,
      showRefresh: true,
        ...
        title: '任务名称',
          ...
          valign: 'middle',
           ...
           formatter:function(value,row,index){  
               ...
               var a = '<a href="DefectDetail.do?bugid='+row.taskid+'">'+ row.taskname+'</a> ';
          ...
          width: '5%',
              formatter:function(value,row,index){  
            	  if(row.tstatus==1){ return "新建";}   	  
            	  if(row.tstatus==2){  return "已解决";  }
            	  if(row.tstatus==3){  return "已关闭";  }
            	  if(row.tstatus==4){  return "打开";  }
            	  if(row.tstatus==5){  return "拒绝";  }
            	  if(row.tstatus==6){  return "重新打开";  }
                   } 
...
////    				关闭   编辑   修复  删除 分配  重新打开   拒绝       	
                	...
                	var  nrbug="拒绝 ";
                	var  hxbug="<a href='#'    onclick=\"showFenPei("+row.bugid+","+row.assigned+")\" >  <font color='#F00'>分配   </font></a>   ";
                	var  nxbug="分配 ";
                	var result ="";
//                	
                  if(row.tstatus==1){  // 新建
