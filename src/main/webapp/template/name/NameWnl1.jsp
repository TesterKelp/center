...
<link rel="stylesheet" href="/module/name/styles/datatimelayout.css" type="text/css" />
	...
	for(i=0;i<42;i++) {
		...
		if(sD>-1 && sD<cld.length) { //日期内
			...
			if(cld[sD].isToday){
				...
				setInterval(function(){with(new Date)document.getElementById("
").innerHTML = (getHours()<10? ('0'+getHours()):getHours())+':'+(getMinutes()<10? ('0'+getMinutes()):getMinutes())+':'+(getSeconds()<10? ('0'+getSeconds()):getSeconds())},1000)
				setInterval(function(){with(new Date)document.getElementById("a15").innerHTML = '（'+'子丑丑寅寅卯卯辰辰巳巳午午未未申申酉酉戌戌亥亥子'.charAt(getHours())+'时'+(getHours()%2==0?'五六七八':'一二三四').charAt(parseInt(getMinutes()/15))+'刻）'},1000)
...
function changeCld() {
	...
	drawCld(y,m);	
	switch (m){
		case 0 : ST.innerHTML = '【 '+STA[0]+kgm+STA[1]+' 】'; a20.innerHTML = '【 '+STA[0]+kgm+STA[1]+' 】';
		break;
		case 1 : ST.innerHTML = '【 '+STA[1]+kgm+STA[2]+' 】'; a20.innerHTML = '【 '+STA[1]+kgm+STA[2]+' 】';
		break;
		case 2 : ST.innerHTML = '【 '+STA[2]+kgm+STA[3]+' 】'; a20.innerHTML = '【 '+STA[2]+kgm+STA[3]+' 】';
		break;
		case 3 : ST.innerHTML = '【 '+STA[3]+kgm+STA[4]+' 】'; a20.innerHTML = '【 '+STA[3]+kgm+STA[4]+' 】' ;
		break;
		case 4 : ST.innerHTML = '【 '+STA[4]+kgm+STA[5]+' 】'; a20.innerHTML = '【 '+STA[4]+kgm+STA[5]+' 】';
		break;
		case 5 : ST.innerHTML = '【 '+STA[5]+kgm+STA[6]+' 】'; a20.innerHTML = '【 '+STA[5]+kgm+STA[6]+' 】';
		break;
		case 6 : ST.innerHTML = '【 '+STA[6]+kgm+STA[7]+' 】'; a20.innerHTML = '【 '+STA[6]+kgm+STA[7]+' 】';
		break;
		case 7 : ST.innerHTML = '【 '+STA[7]+kgm+STA[8]+' 】'; a20.innerHTML = '【 '+STA[7]+kgm+STA[8]+' 】';
		break;
		case 8 : ST.innerHTML = '【 '+STA[8]+kgm+STA[9]+' 】';  a20.innerHTML = '【 '+STA[8]+kgm+STA[9]+' 】'; 
		break;
		case 9 : ST.innerHTML = '【 '+STA[9]+kgm+STA[10]+' 】'; a20.innerHTML = '【 '+STA[9]+kgm+STA[10]+' 】';
		break;
		case 10 : ST.innerHTML = '【 '+STA[10]+kgm+STA[11]+' 】'; a20.innerHTML = '【 '+STA[10]+kgm+STA[11]+' 】';
		break;
		default : ST.innerHTML = '【 '+STA[11]+kgm+STA[0]+' 】';  a20.innerHTML = '【 '+STA[11]+kgm+STA[0]+' 】';
	}
}
...
function mOvr(v) {
	...
	if(sObj.innerHTML!='') {
		...
		for(i=0;i<12;i++){
			rObj=eval('RC'+ i);
			rObj.style.background = '#ffffff';
			if(Zhi[i]==cld[d].cDay.substring(1,2)){ 
				rObj.style.background = '#66ccff';
			//	alert(i);
			}
			if(Zhi[i]==RCT){ rObj.style.background = '#fc9a29';}
			
			rObj1=eval('m'+ i);
			rObj1.style.background = '#990033';
			if(Zhi[i]==cld[d].cDay.substring(1,2)){ 
				rObj1.style.background = '#fc9';
				$("#w3").val(i); 
			}
		...
		for(i=0;i<12;i++) {
			...
			a =a+(cld[d].dGz).substring(i*2,(i+1)*2)+"," ;
			SCObj1=eval('N'+ i);
			SCObj1.innerHTML = (cld[d].dGz).substring(i*2,(i+1)*2);
			N.innerHTML =  cld[d].sDay + "日时辰：";
			$("#w1").val(a); 
		...
		for(i=0;i<12;i++) {
			...
			SJObj1=eval('NN'+ i);
			SJObj1.innerHTML = (cld[d].sgz).substring(i*3+1,(i+1)*3);
			b =b+(cld[d].sgz).substring(i*3+1,(i+1)*3) +","; 
			$("#w2").val(b); 
		...
		a1.innerHTML =cld[d].xsfw;
		a2.innerHTML =cld[d].fsfw;
		a3.innerHTML =cld[d].csfw;
		...
		a4.innerHTML = nymdw;
		a5.innerHTML = days;
		a6.innerHTML = '<font size="7" face="Arial Black">' + cld[d].sDay + '</font>';

		a7.innerHTML = Lunard+'&nbsp;'+zhou;
		a8.innerHTML = cYMD;
		a9.innerHTML = '<table width="100%"><tr><td rowspan="3" width="30px" align="center" bgcolor="#FEF693"><font color="red">节<br><br>日</font></td><td height="36px" bgcolor="#FEF693"><FONT color="blue">&nbsp;&nbsp;'+ cld[d].solarFestival + '</FONT></td></tr><tr><td height="22px" bgcolor="#FEF693"><FONT color="red">&nbsp;&nbsp;' + cld[d].lunarFestival + '</FONT></td></tr><tr><td height="22px" bgcolor="#FEF693"><FONT color="#006600">&nbsp;&nbsp;' + cld[d].solarTerms+'</FONT></td></tr></table>';
		a11.innerHTML = jy;
		...
		var r =cld[d].solarFestival+ ';' + cld[d].solarTerms ;
		$("#s6").val(r);
		$("#s7").val(jy);
...
<div id="my"  style="border:1px solid #00a0e9;margin:30px">


<div style="float:right;width:20%;margin:40px"> 
说明：<br>

黄道、黑道共有十二辰，依次是：一青龙、二明堂、三天刑、四朱雀、五金匮、六天德、七白虎、八玉堂、九天牢、十玄武、十一司命、十二勾陈。<br>
其中，青龙、明堂、金匮、天德、玉堂、司命为黄道，主吉；<br>
天刑、朱雀、白虎、天牢、玄武、勾陈为黑道，主凶。<br>
利用黄道、黑道可以推出黄道吉日、黄道吉时。<br>
青龙：消息吉。	明堂：吉。 <br>
金匮：财星吉。	天德：官贵吉。 <br>
玉堂：吉。	司命：官长吉。 <br><br>
天刑：驿马强动。	朱雀：口舌。  <br>
...
<div style="padding-left:100px;float:left;width:25%">
<div id="a4"  align="center" style="font-size:15px;background-color:red; margin:0 auto"></div>
...
<input type="text" id="w1"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">
<input type="text" id="w2"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">
<input type="text" id="w3"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">


<input type="text" id="x1"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">
<input type="text" id="x2"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">
<input type="text" id="x3"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">



<input type="text" id="s1"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">
<input type="text" id="s2"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">
<input type="text" id="s3"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">


<input type="text" id="s4"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">
<input type="text" id="s5"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">
<input type="text" id="s6"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">

<input type="text" id="s7"   siz="15" style="margin-bottom: 0px; margin-top: 8px; width:400px">

<br>

<div style="clear:both;"></div>

<div  align="center" >黄道吉日</div>


<table class="ttable">
<tr  class="ttr" >
<td  class="ttd">日辰吉凶</td>
<td  class="ttd" id="m0">青龙</td>
<td  class="ttd" id="m1">明堂</td>
<td  class="ttd" id="m2">天刑</td>
<td  class="ttd" id="m3">朱雀</td>
<td  class="ttd" id="m4">金匮</td>
<td class="ttd"  id="m5">天德</td>
<td class="ttd"  id="m6">白虎</td>
<td class="ttd"  id="m7">玉堂</td>
<td class="ttd"  id="m8">天牢</td>
<td class="ttd"  id="m9">玄武</td>
<td class="ttd"  id="m10">司命</td>
<td class="ttd"  id="m11">勾陈</td>
</tr>




<tr style="border:1px solid #00a0e9;margin:30px;">
<td style="border:1px solid #00a0e9;margin:30px;width:250px" id="N">    </td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N0">戊子(子时)</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N1">己丑(丑时)</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N2">庚寅</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N3">辛卯</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N4">壬辰</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N5">癸巳</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N6">甲午</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N7">乙未</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N8">丙申</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N9">丁酉</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N10">戊戌</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="N11">己亥</td>
</tr>


<tr style="border:1px solid #00a0e9;margin:30px;">
<td style="border:1px solid #00a0e9;margin:30px;width:250px" id="NN">时辰吉凶    </td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN0">戊子(子时)</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN1">己丑(丑时)</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN2">庚寅</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN3">辛卯</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN4">壬辰</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN5">癸巳</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN6">甲午</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN7">乙未</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN8">丙申</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN9">丁酉</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN10">戊戌</td>
<td style="border:1px solid #00a0e9;margin:30px;width:150px" id="NN11">己亥</td>
</tr>
</table>
