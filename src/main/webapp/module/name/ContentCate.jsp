      ...
      <div class="subnavList">
        ...
        <ul>
                  ...
                  <c:forEach items="${lst}" var="a" varStatus="status">
					<li><span style="PADDING-RIGHT: 10px; FLOAT: right;">[2019-09-19]</span> <a href="/name/ContentCateDetail.do?id=${a.articleid}">${a.title}</a></li>
		          </c:forEach>
...
<div class="ep-pages">
				...
				<c:if test="${pagenumber != 1}">     
                   <a href="ContentByCateId.do?id=6&pagenumber=1" target="_self" >首页</a>
                 </c:if> 
			<c:forEach var="i" begin="1" end="${totalpage}" step="1">   
 				...
 				<c:if test="${pagenumber == i}">     
                    <a href="ContentByCateId.do?id=6&pagenumber=${i}" target="_self" class="current">${i}</a>
                 </c:if>   
                  <c:if test="${(pagenumber != i)}">     
                   <a href="ContentByCateId.do?id=6&pagenumber=${i}" target="_self">${i}</a>
                 </c:if> 
    		  </c:forEach>  
			    <c:if test="${pagenumber != totalpage}">     
                 <a href="ContentByCateId.do?id=6&pagenumber=${totalpage}" target="_self">尾页</a>
                 </c:if>
